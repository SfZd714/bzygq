...
![images/ESJHbmq6roPrkCxBmuUcR9cknuf.png](../_images/8d7f08e6eb604eaab1512aa9839c1705.png)

images/ESJHbmq6roPrkCxBmuUcR9cknuf.png

Java运行时数据区（也称为Java内存模型或JVM内存结构）是Java虚拟机（JVM）在执行Java程序时使用的内存布局。它被划分为几个不同的部分，每个部分都有其特定的用途和生命周期。以下是Java运行时数据区的主要组成部分：

1. **堆（Heap）**： 这是Java程序中所有类实例和数组对象的存储区域。堆是由垃圾收集器管理的，用于动态分配内存。所有的线程共享这个堆空间。堆被进一步细分为新生代（Young Generation）、老年代（Old Generation）和永久代/元空间（PermGen/Metaspace）。新生代又可以分为Eden空间和两个Survivor空间。
2. **方法区（Method Area）**： 方法区是所有线程共享的内存区域，用来存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码等数据。在HotSpot JVM中，方法区有时被称为“永久代”（PermGen），但在Java 8及以后版本中，已经被“元空间”（Metaspace）取代，后者使用本地内存而不是固定大小的内存区域。
3. **栈（Stack）**： 每个线程创建时都会有一个私有的栈，用于存储局部变量表、操作数栈、动态链接、返回地址等信息。每个方法调用都会在线程栈中创建一个栈帧（Frame），当方法调用结束时，对应的栈帧也会被弹出。
4. **程序计数器（Program Counter Register）**： 程序计数器是一块较小的内存空间，它的作用可以看作是当前线程所执行的字节码指令的索引指示器。对于Java来说，每个线程都有自己的程序计数器，它是线程私有的。
5. **本地方法栈（Native Method Stack）**： 与Java方法使用的Java栈相对应，本地方法栈为本地方法服务。具体实现可能依赖于具体的JVM实现；有些JVM可能会将它和Java栈合并。
6. **直接内存（Direct Memory）**： 直接内存并不是JVM运行时数据区的一部分，但它仍然经常被使用。它指的是通过java.nio.ByteBuffer.allocateDirect分配的内存，这部分内存是在JVM之外的，因此不受JVM的垃圾回收机制控制。然而，由于它可以绕过JVM的堆内存限制，并且能够提高I/O操作的性能，所以在某些场景下非常有用。

## 方法区

### 什么是方法区？

![images/KGcTbGwKooR7rkxpRPwcmAO2nmg.png](../_images/597e7dfc4531416689d218d8654bd4ef.png)

images/KGcTbGwKooR7rkxpRPwcmAO2nmg.png
...
- **释放：** 关闭JVM将释放方法区的内存空间。 方法区是用于存储类信息、常量池、静态变量等数据的区域，对于Java虚拟机的正常运行和类加载等都具有重要作用。

#### 方法区的前世今生

![images/XzUObfWTyoMamzxjxAGcLm6qnec.png](../_images/6e60ee282c854b899d795419ddab8cc8.png)

images/XzUObfWTyoMamzxjxAGcLm6qnec.png
...
![images/B2ULbG6oTo7zsZxuYcccOl92nPf.png](../_images/4bc0a674cefb4a808fc5f88aa99b8494.png)

images/B2ULbG6oTo7zsZxuYcccOl92nPf.png
...
![images/OEZVb97dUoeolyxqLAucvhj3n6g.png](../_images/4786a567594844099b9b1b1645fd5518.png)

images/OEZVb97dUoeolyxqLAucvhj3n6g.png
...
永久代是Java虚拟机堆内存的一部分，主要用于存储类的元数据信息。包括类的结构、方法、字段、注解等。在Java 8及之前的版本，永久代有一定的固定大小，当存储的类信息较多时容易导致永久代溢出。此时可能会出现OutOfMemoryError: PermGen space错误。
...
- \-XX:MetaspaceSize: 用于设置初始的元空间大小。对于64位的服务器JVM，默认值为21MB。这是初始的高水位线，一旦达到这个水位线，Full GC将被触发，卸载不再存活的类，然后重置高水位线。新的高水位线取决于GC释放了多少空间。如果释放的空间不足，适当提高该值，不超过MaxMetaspaceSize。如果释放的空间过多，则适当降低该值。
...
![images/Gr2Ubn4lgonp33xhkYjcY60gnoe.png](../_images/a0fc1500c43e4d3298bd567b15275f73.png)

images/Gr2Ubn4lgonp33xhkYjcY60gnoe.png
...
1. **类型的完整有效名称：** 例如，对于类java.lang.String，存储的是完整的包名和类名。
2. **类型直接父类的完整有效名：** 对于接口或是java.lang.Object，它们没有直接的父类。
3. **类型的修饰符：** 存储类型的修饰符，可能包括public、abstract、final等的某个子集，用于描述该类型的访问权限、抽象性质和是否可以被继承等信息。
4. **类型直接接口的一个有序列表：** 存储类型实现的接口列表，有序表示接口的顺序，这对于多接口实现的情况很重要。 这些信息使得虚拟机能够准确地了解每个加载类型的结构和关系，包括其继承层次、实现接口、访问权限等。这对于类的加载、链接和运行时行为都具有重要作用。
...
- **字节码文件中包含了常量池：** 字节码文件的常量池是在编译期生成的，包含了类中的常量、类名、字段名、方法名等信息。它是一种静态的、不可变的常量池，保存了在编译时确定的字面量和符号引用。字节码文件中的常量池在类加载时被加载到方法区的运行时常量池中，供虚拟机执行时使用。 总体而言，字节码文件中的常量池是类的静态结构的一部分，而运行时常量池是类在运行时动态生成的。运行时常量池的动态性使得它可以包含更多在运行期间生成的信息，而字节码文件中的常量池则提供了类的静态元数据。这两者的结合使得虚拟机能够更好地支持动态语言特性和类的动态变化。
...
public static final String DEFAULT\_NAME = "John Doe"; 
}

在上述例子中，MAX\_VALUE 和 DEFAULT\_NAME 都是全局常量，它们的值在编译时确定，并且在运行时不可修改。

##### **即时编译器（JIT）编译后的代码缓存：**

方法区在 Java 虚拟机中不仅存储类的元数据信息，还承担了即时编译器（JIT）生成的本地机器代码的存储任务。即时编译器是一种优化技术，它将部分热点代码（经常执行的代码路径）编译成本地机器代码，以提高程序的执行性能。具体而言，方法区中存储了以下内容：

1. **本地机器代码：** JIT 编译器将部分 Java 字节码编译成本地机器代码，这样可以更高效地执行。这些本地机器代码通常涉及程序的热点部分，即频繁执行的代码段。
2. **代码缓存：** 编译后的本地机器代码被缓存，以避免重复编译相同的代码。缓存机制有助于减少重复工作，提高程序的运行效率。当相同的代码段再次执行时，虚拟机可以直接使用缓存中的本地机器代码，而不必重新进行编译。 通过将热点代码编译成本地机器代码，并进行缓存，JIT 编译器在运行时动态地优化程序的性能。这种动态编译和优化的机制使得虚拟机能够更好地适应不同的程序运行情境，提高程序的执行效率。方法区的设计使得它能够承担这一重要角色，为 Java 程序的性能提供支持。
...
- transient: 表示该字段不会被序列化。 这些信息使得虚拟机能够了解每个字段的名称、类型、访问权限、是否为静态字段以及其他修饰信息，从而能够正确地处理字段的访问和操作。这对于类的实例化、访问字段以及字段的初始化等操作都具有重要作用。
...
1. **方法的字节码（bytecodes)、操作数栈、局部变量表及大小：** 这些信息用于存储方法的具体实现，包括方法体的字节码指令、操作数栈的状态、局部变量表的内容及其大小。这些信息对于虚拟机执行方法体的字节码指令非常关键。
2. **异常表：** 用于存储方法中可能抛出的异常的处理信息，包括每个异常处理块的开始位置、结束位置、代码处理在程序计数器中的偏移地址以及被捕获的异常类的常量池索引。这些信息帮助虚拟机在出现异常时进行适当的处理。 这些方法信息使得虚拟机能够正确地加载、解析和执行方法，处理方法的调用、返回值、异常处理等操作。
...
3. **缺少Metaspace的垃圾回收：** 在Java 8及之后的版本，Metaspace不再有垃圾回收的概念，因此如果动态生成的类或加载的类不再使用，Metaspace也不会自动释放相关内存。这可能导致Metaspace逐渐增长，最终耗尽内存。 解决Metaspace的OOM：
4. **调整Metaspace大小：** 可以通过JVM参数（如-XX:MaxMetaspaceSize）来调整Metaspace的大小。增大Metaspace的内存限制可能会缓解OOM，但需要确保整个系统内存足够。
5. **优化类加载和卸载：** 确保应用程序中只加载必要的类，避免不必要的类加载。使用工具监控类加载和卸载的情况，查看是否有无用的类没有被卸载。
6. **检查动态生成类的逻辑：** 如果应用程序中有动态生成类的逻辑，确保生成的类数量有限，并且可以被及时卸载。
7. **监控Metaspace使用情况：** 使用JVM的监控工具，如VisualVM或JConsole，来监控Metaspace的使用情况，及时发现潜在的OOM问题。 总的来说，Metaspace的OOM问题需要特别关注动态生成类的逻辑和类加载的行为，以及及时调整Metaspace的内存大小。
...
3. 该类对应的java.lang.Class对象没有在任何地方被引用，无法通过反射访问该类的方法。 Java虚拟机被允许对满足上述三个条件的无用类进行回收，这里强调的是“被允许”，而不是像对象一样没有引用就必然回收。
...
我们知道当程序运行的时候，需要一个”栈“的数据结构来完成对源代码转译而来的操作指令（JVM的操作指令）的压栈和出栈等操作，来实际运行程序。在Java中程序的运行一般是以线程为单位的，而虚拟机栈就是Java线程专属的栈空间。每个线程创建之后都会单独申请自己独立的一个栈空间，每个方法的调用都会对应着一个**栈帧**，栈帧包含局部变量表、操作数栈、动态连接、方法出口等。

![images/K5o3bAwpdoFi8kxjJvic4XN4nsg.png](../_images/484fd591f46f436eaa041573605ad35d.png)

images/K5o3bAwpdoFi8kxjJvic4XN4nsg.png

在 Java 虚拟机中，每个线程存在着一个虚拟机栈，每个方法的执行都对应着一个栈帧，栈帧的入栈和出栈过程，以及栈帧之间的切换和返回值传递。栈和栈帧是实现方法调用和执行的重要机制：

1. **栈与栈帧：**

- **栈：** Java 虚拟机栈是线程私有的，用于存储线程执行方法时的**局部变量**、**操作数栈、动态链接、方法出口**等信息，每个线程都有自己的 Java 虚拟机栈。
- **栈帧：** 每个方法的执行对应一个栈帧。栈帧包含了**局部变量表、操作数栈、动态链接、方法出口等信息**。每次方法调用时都会创建一个新的栈帧，该栈帧被推入虚拟机栈，成为当前活动栈帧。
...
![images/UMjKbgRHlos11zxQyPWcsWwanCF.png](../_images/2ac155ba1e5f44f1a74697a171508557.png)

images/UMjKbgRHlos11zxQyPWcsWwanCF.png
...
**栈帧的组成**包括了**局部变量表、操作数栈、动态连接、方法返回地址等信息**。在编译代码时，局部变量表和操作数栈的大小已经确定，并写入方法表的 Code 属性中。因此，栈帧所需的内存不受程序运行期变量数据的影响，仅取决于虚拟机的实现。

就**栈帧的状态**来说，一个线程中可能存在很多方法调用，方法之间相互调用，形成一个方法调用链。对于JVM来说，只有当前栈帧才是有效的，被称为当前栈帧（Current Stack Frame）。当前栈帧关联的方法称为当前方法（Current Method）。JVM只针对当前栈帧进行操作，执行引擎运行的所有字节码指令都是针对当前栈帧进行的。
...
1. **存储局部变量：** 局部变量表用于存储方法中的局部变量，包括方法参数和在方法体内部定义的局部变量。系统不会为局部变量赋予初始值，与实例变量和类变量不同，局部变量在准备阶段不会被赋予初始值。
2. **数据类型：** 局部变量表中的每个槽位可以存储一个数据，而数据的类型可以是各种基本数据类型（如int、float、double、long等）以及对象引用。局部变量表的存储以变量槽（Slot）为最小单位。在32位虚拟机中，一个Slot可以存放32位（4字节）以内的数据类型，包括boolean、byte、char、short、int、float、reference和returnAddress等八种类型。对于64位长度的数据类型（如long和double），虚拟机以高位对齐方式为其分配两个连续的Slot空间，相当于将一次long和double数据类型读写分割成两次32位读写。并且，Slot是可以重用的，当Slot中的变量超出作用域时，下一次分配Slot时会覆盖原有的数据。对于对象的引用存储在Slot中，会影响垃圾回收，即如果被引用，对象将不会被回收
3. **编译时确定：** 局部变量表的容量在编译时就已经确定了，因为它与方法的代码结构直接相关。每个局部变量表的槽位可以存储一个数据，而槽位的索引从0开始。
4. **作用域：** 局部变量表的作用域仅限于方法体内，当方法执行结束时，局部变量表的内容会被销毁。
5. **访问方式：** 局部变量表中的局部变量可以通过索引访问。例如，iload\_1 指令表示将索引为1的局部变量加载到操作数栈中。 以下是add方法和相应的字节码，说明了局部变量表的使用：
...
使用javac命令编译之后相应的字节码包含以下的指令（具体编译字节码的方法可以[参考这里](https://mp.weixin.qq.com/s?__biz=MzkzMjI0MDY3Mw==&mid=2247484211&idx=1&sn=ab014d1d3df5cb478a12d54cce6fd488&chksm=c25f8f7df528066b0d75f670f6668d3cef5d0eb0a2d24935ac48fd5061694a715d4ead18d701#rd)）：
...
当add方法执行之前，JVM会将add方法构造成一个栈帧推入main线程虚拟机栈中，而参数a和b会作为局部变量，被放置到局部变量表当中。而iload\_1和iload\_2会分别从局部变量表中读取第一个和第二个元素进行计算。
...
1. **栈的结构：** 操作数栈是一个后进先出（LIFO）的栈结构，类似于数据结构中的栈。最后压入栈的元素最先被弹出。
2. **存储操作数：** 操作数栈主要用于存储方法执行时需要操作的数据，包括方法参数、中间计算结果以及方法返回值。
3. **字节码执行：** 在执行方法时，字节码中的各种指令会涉及到对操作数栈的读取和写入操作。例如，将局部变量加载到操作数栈、进行算术运算、方法调用等都涉及到对操作数栈的操作。
4. **临时存储：** 操作数栈的内容是临时性的，它在方法执行期间用于暂存数据。一旦方法执行结束，操作数栈的内容也随之被销毁。 如下面的Java代码，这里简单实现了一个add的方法：
...
iload\_1 // 将局部变量 a 推入操作数栈 
iload\_2 // 将局部变量 b 推入操作数栈 
iadd // 从操作数栈中弹出两个值，相加，将结果推入操作数栈 
istore\_3 // 将操作数栈的结果存储到局部变量 result 
ireturn // 将结果从方法中返回

在这个例子中，操作数栈用于存储局部变量 a 和 b 的值，并在 iadd 指令中执行加法操作，将结果存储到局部变量 result 中。ireturn 指令将最终结果从方法中返回。整个过程中，操作数栈充当了中间计算结果的存储区域，具体如下列图：

![images/CpPXbTpWxonI2kxLCDTcT3oBnYe.png](../_images/169a8f522dc04963aab631565b2c7403.png)

images/CpPXbTpWxonI2kxLCDTcT3oBnYe.png

- iload\_1 和 iload\_2 会分别将局部变量表中索引为1和索引为2的变量压入操作数栈，为后面进行运算做准备。

![images/RhsPbdgDuoOcDRxYB3Vcd2MHnYd.png](../_images/7da200eed519471c81c74e25c3b0d301.png)

images/RhsPbdgDuoOcDRxYB3Vcd2MHnYd.png
...
#### JVM栈与Stack Over Flow分析

##### 运行Java代码时设置栈大小方法

在 Java 虚拟机中，可以使用 -Xss 或 -XX:ThreadStackSize 参数来设置每个线程的栈大小。以下是两种常见的设置方式：

1. **使用** \-Xss\*\* 参数：\*\* 将设置每个线程的栈大小为 512KB

java -Xss512k test.java

1. **使用** \-XX:ThreadStackSize\*\* 参数：\*\* 设置每个线程的栈大小为 1024 字节

java -XX:ThreadStackSize=1024 YourJavaProgram

需要注意的是，这些参数的设置可能会受到具体的 JVM 实现和版本的影响，在实际应用中，通常不需要手动设置栈大小，Java 虚拟机会根据默认规则进行分配。

##### 执行会导致Stack Over Flow的Java 源代码文件

###### 设置线程栈大小为256kb，复现stack over flow错误

- 一段Java代码：编写一个会导致 Stack Overflow 的 Java 代码如下：
...
System.out.println("----------------------"); 
System.out.println("Initial depth: 1"); 
System.out.println("----------------------"); 
System.out.println("Setting stack size to 256KB"); 
System.out.println("----------------------"); 
// 使用 -Xss 参数设置栈大小 
// 请注意，过小的栈大小可能导致 Stack Overflow 错误 
// 在实际应用中，设置合适的栈大小是非常重要的 
// 在正常情况下，不需要手动设置栈大小 
System.out.println("Running recursiveMethod..."); 
recursiveMethod(1); 
} catch (Throwable e) { 
System.out.println("----------------------"); 
...
在这个例子中，recursiveMethod 方法是一个递归方法，每次递归都会打印递归深度。在 main 方法中，我们尝试运行这个递归方法，并设置较小的栈大小。

- 编译 Java 代码：在当前目录生成 StackOverflowExample.class 文件
- 编译 Java 源代码文件javac StackOverflowExample.java
- 运行 Java 代码并设置栈大小：
- 运行 Java 程序并设置栈大小为 64KBjava -Xss256k -cp . your/path/StackOverflowExample，-Xss256k 参数用于设置每个线程的栈大小为 256KB，上面的Java代码中会无限递归地创建recursiveMethod（），并且这个方法永远不会返回，指导JVM栈溢出，出现Stack Overflow 错误。

##### 分析Stack Overflow

在上面的示例代码操作中，mian线程的栈会不断的压入方法recursiveMethod的栈帧，由于该函数是无限递归的，所以永远不会被返回。因此，main线程中的栈帧永远不会被释放，知道栈帧占满虚拟机栈空间，导致Stack Overflow错误，上述操作的执行结果如下：

从上面的执行结果我们可以看出，栈的最大深度为1640，加上main方法的栈帧，main线程的栈中一共有1641个栈帧。而这段代码运行时的栈大小设置为了256 kb，单个栈帧的大小为 256\*1024/1641 = 160 Bytes。 实际上，当Java方法中的局部变量增多之后，栈帧所占用的空间会逐渐增加，所以一般栈帧的大小在几百字节～几KB左右。而且栈帧也一般会随着方法的返回而自动地被JVM所卸载和释放。
...
我们知道Java是一种跨平台的语言，也就是说同一段Java代码在所有的计算机平台上的运行表现都是一样的。一般的PC平台可以分为Window、Macos和Linux等，而移动端平台也分为IOS、Android，但是，Java代码在这些所有的平台上的运行表现都是一致的，这是怎么做到得呢？

实际上，Java本身是运行在虚拟机上的，而Java虚拟机（Java Virtual Machine，JVM）则是Oracle或者其它的HotSpot 厂商根据JVM标准开发的不同平台的虚拟机，从而将Java与平台解耦。JVM为我们屏蔽了平台的特性同时进行了抽象，使得Java代码可以一次编译处处运行。例如，我们在Java中创建一个线程只需要调用如下Java代码：

Thread thread = new Thread();

而实际上，不同平台创建线程的方式各不相同，其底层的操作系统也提供了不同的API来进行线程创建，如下：
...
pthread\_t thread; 
int result = pthread\_create(&thread, NULL, MyThreadFunction, NULL);

可以看到，在linux平台上和window平台上创建线程的方法完全不同，而这些方法实际上是我们在这个平台上创建线程的唯一方法，是由操作系统提供的。因此，对于各个平台来说，运行的根本不是我们前面写的Java代码，而是这些系统系统的接口和方法。

我们的操作系统，无论是windows还是linux都是由C++代码书写而成的，因此，这些系统api一般都是C++的函数，所以我们后面说的native方法一般也就是c++方法。Java虚拟机的作用就是相当于一个包装层，去把这些Java代码翻译称为平台相关的代码或者二进制文件，然后给相应平台执行。

因此，Java只需要针对不同平台实现各自的满足要求的Java虚拟机，就可以实现一次编译处处运行。

#### 什么是本地方法？

接着，我们就可以说说什么是Java的本地方法了。在Java中，本地方法（Native Method）是指由Java语言以外的语言编写的方法，这些方法的实现是由本地代码（通常是C或C++等语言编写的和平台相关的代码）编写的，并且可以通过Java本地接口（Java Native Interface，JNI）在Java程序中调用的方法。

Java的设计初衷是实现跨平台性，使得Java程序可以在不同的操作系统和硬件平台上运行。很多任务和方法都需要直接与底层系统交互，使用本地代码可以提高性能或者利用特定平台的功能。下面是使用本地方法的一般流程如下：

1. **编写本地方法实现：** 在本地代码中编写实现某个功能的方法。这通常是使用C或C++等语言完成的，这些C/C++语言编译成的代码通常跟特定平台相关。
2. **生成本地库：** 将本地代码编译成共享库或动态链接库（如.dll、.so等），这称为本地库。
3. **声明本地方法：** 在Java中声明一个本地方法，使用native关键字，并提供一个外部声明，告诉编译器该方法的实现在本地库中。
4. **使用JNI加载本地库：** 在Java程序中使用JNI加载本地库，以便在运行时能够调用本地方法。
5. **调用本地方法：** 在Java代码中调用本地方法，通过JNI桥接到本地库中的实现，从而完成与本地代码的交互。 例如，以下是一个简单的示例：
...
在这个示例中，callNativeMethod方法是一个本地方法，它的实现在本地库中。System.loadLibrary("NativeLibrary");用于加载名为NativeLibrary的本地库。

需要注意的是，使用本地方法会降低Java程序的可移植性，并增加了与底层系统交互的复杂性。因此，通常建议在不是必要的情况下尽量避免使用本地方法，而选择纯Java实现。

#### **本地方法栈**

前面介绍了什么是本地方法，即与平台、硬件相关的方法，一般使用C/C++语言实现，能够为Java跨平台功能提供支持。与此同时，Java代码本身为了提升某些模块的性能，也会通过JNI的方法去调用本地的C++方法等，这些C/ C++方法在编译之后，一般会被打成本地的.so或者.dll库来供Java代码使用。例如，一般的手机APP调用的美颜算法等，出于效率考虑，美颜算法就会使用C++实现SDK，然后给上层调用。

因此，Java的本地方法栈（Native Method Stack）就是专门用于管理本地方法的调用的栈，本地方法栈和Java虚拟机栈在内存结构上是分离的。本地方法栈通常包含本地方法的调用信息、局部变量以及用于操作数栈的相关信息，主要有以下特点：

- 本地方法栈是线程私有的，每个线程都有自己的本地方法栈
- 本地方法的堆内存大小可以被实现为固定或可动态扩展的，一般C和C++代码的堆内存需要程序员手动进行管理
- 如果本地方法栈请求的栈容量超过了允许的最大值，将导致StackOverflowError异常
- 在动态扩展的情况下，如果尝试扩展栈时无法获得足够的内存，或者在创建新线程时无法分配足够的内存以创建对应的本地方法栈，将导致OutOfMemoryError异常 线程调用本地方法时，进入一个全新且不再受虚拟机限制的领域，拥有相同的权限。本地方法通过本地方法接口可以访问虚拟机内部的运行时数据区，具有直接使用本地处理器中的寄存器的能力，可以直接从本地内存的堆中分配任意数量的内存。

#### 总结

总而言之，为了跨平台Java针对不同的平台和硬件设备实现了相应的Java虚拟机，用来屏蔽相关平台的特性，使得Java代码能够一次编译处处运行。因此，Java线程本身就会调用很多的平台相关的方法，这些方法一般是使用C/C++等语言实现的。而本地方法栈就是这些本地方法的执行栈，每个线程都会有一个独立的本地方法栈，用来调用本地平台功能。

## JVM程序计数器

### 什么是程序计数器？

![images/HHksbtsSgoQhQVx5tqIcyu9Nnag.png](../_images/61af5bfc5b604122bce597eb88e6020a.png)

images/HHksbtsSgoQhQVx5tqIcyu9Nnag.png
...
3. **支持方法调用和返回：** 在Java程序中，方法调用和返回都依赖于程序计数器。程序计数器保存方法调用的返回地址，以便方法执行完毕后能够正确返回到调用点。 程序计数器是JVM中唯一一个在多线程执行时不会发生线程安全问题的区域。这是因为每个线程有自己独立的程序计数器，不会被其他线程访问或修改。总体而言，JVM中的程序计数器在支持Java程序的运行时起到了关键的作用，它是实现线程切换和方法调用等功能的重要组成部分。

### 程序计数器执行流程

![images/Yif7bNu0Yoc7GBxedIVctsYanfb.png](../_images/b9b08b2187694c4f95f2f0ce82a0049f.png)

images/Yif7bNu0Yoc7GBxedIVctsYanfb.png
...
1. **iload\_1:** 程序计数器记录下将执行的指令的地址偏移0。iload\_1指令执行，将局部变量表中索引为1的局部变量（例如，整数变量 a）加载到操作数栈上。
2. **iload\_2:** 程序计数器更新为下一条指令的地址。iload\_2指令执行，将局部变量表中索引为2的局部变量（例如，整数变量 b）加载到操作数栈上。
3. **iadd:** 程序计数器继续更新。iadd指令执行，从操作数栈中弹出两个整数，相加并将结果推入操作数栈。
4. **istore\_3:** 程序计数器继续更新。istore\_3指令执行，将操作数栈顶的整数值存储到局部变量表中索引为3的位置（例如，整数变量 result）。
5. **iload\_3:** 程序计数器再次更新。iload\_3指令执行，将局部变量表中索引为3的局部变量的值加载到操作数栈上。
6. **ireturn:** 程序计数器继续更新。ireturn指令执行，将结果作为方法的返回值，并结束方法的执行。 在整个执行过程中，程序计数器始终记录着当前线程即将执行的指令的地址。这保证了指令的有序执行，同时在方法调用、返回和异常处理等情况下，程序计数器都发挥了关键的作用。
...
private static List objectList = new ArrayList<>(); 
public static void main(String\[\] args) { 
for (int i = 0; i listeners = new ArrayList<>(); 
public static void main(String\[\] args) { 
...
for (int i = 0; i objectCache = new HashMap<>(); 
public static void main(String\[\] args) { 
// 缓存对象 
cacheObjects(); 
// 在程序的其他部分可能未正确清理缓存 
