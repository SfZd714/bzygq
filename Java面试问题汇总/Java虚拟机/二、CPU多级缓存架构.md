...
在实际工作中或者面试过程中，多线程是很多同学绕不开的一个话题，即使是在央企国企的技术岗，多线程也是属于必问的问题。

但是，从顶层或者业务层去理解为什么要同步、异步、多线程操作、并发问题和锁这些概念会非常复杂，如果我们换一条思路直击问题根本，多线程问题的实质其实就是由于三级缓存架构造成的数据一致性问题。

所以了解CPU的缓存架构，对于了解Java多线程和JVM相关的一些东西是非常有帮助的。
...
![images/M5MzbEZZToxBrex6t5JcA4rHnZb.png](../_images/b1d2adc8be5f4611859aa8a1fcad14d1.png)

images/M5MzbEZZToxBrex6t5JcA4rHnZb.png
...
从缓存的存储空间来说从大到小依次为：内存 > L3 Cache > L2 Cache > L1 Cache > 寄存器。从缓存的存取速度上来说，从快到慢依次为：寄存器 > L1 Cache > L2 Cache > L3 Cache > 内存，如下文所示：
...
![images/PWn0bfJdfoLJXrxpJEscQsoNnne.png](../_images/866ab9e40fa840e2875d10ce962a81f5.png)

images/PWn0bfJdfoLJXrxpJEscQsoNnne.png
...
![images/VoQJb58pgosg1ox0SslcQjp8nUe.png](../_images/e4ac22783d324915b3d11f22b5e2e1db.png)

images/VoQJb58pgosg1ox0SslcQjp8nUe.png

常用的缓存替换策略是**最近最少使用算法**（Least Recently Used，**LRU**），可以给每个缓存行设置一个参数记录其访问次数，每次需要淘汰缓存行的时候使用频率最低的缓存行被淘汰。

LRU算法的实现方式很多，最常用的是——**位矩阵。**位矩阵会定义一个行、列都与缓存路数（way）相同的矩阵，当访问某个way对应的缓存行的时候，会将该路对应的所有行置为1，再将该way对应的所有列都置为0。那么，最近最少使用的缓存行所对应的矩阵行当中的个数最少，就会最先被替换出去。
...
4. 功能：寄存器用于存储指令执行过程中的数据和中间结果；缓存用于缓存常用的数据和指令，以提高数据访问效率。 总体而言，寄存器和缓存在计算机体系结构中扮演不同的角色。寄存器用于快速存取和处理数据，而缓存则用于提供更快的数据访问速度，减少CPU对主内存的访问次数，提高计算机系统的整体性能。

##### 逻辑电路上区别

1. **寄存器逻辑电路结构：**

![images/Ph8Gb1Mk5o0BQpxjR6AcZwHZnVe.png](../_images/2687603997334f47baa4ccd786a45213.png)

images/Ph8Gb1Mk5o0BQpxjR6AcZwHZnVe.png
...
- 缓存的主要功能是通过在CPU和主内存之间提供快速的数据访问，减少对主内存的访问次数，提高数据访问效率。 总结起来，寄存器的逻辑电路结构相对简单，主要由触发器构成，用于存储和暂存数据。而缓存的逻辑电路结构更加复杂，包括标签、索引、块和数据存储器等组成部分，用于提供快速的数据访问和有效的缓存策略。

#### 总线和总线锁

![images/M7yRbzkKoo6M7fxiwVFcrfa7nrc.png](../_images/1f154a2c818c4dd6b6f92fead8aeb5f9.png)

images/M7yRbzkKoo6M7fxiwVFcrfa7nrc.png

##### 总线（Bus）

从它的英文名字可以看出，就像个巴士一样，用来运输计算机各个硬件设备之间的数据。 实际上总线是计算机内部用于传输数据、地址和控制信号的硬件设备，连接了计算机内部的各个部件，如中央处理（CPU）、内存、输入输出设备等。总线主要分为数据总线、地址总线和控制总线几类：
...
总线锁是一种处理器级别的锁机制，用于实现多处理器系统中的原子操作或互斥访问共享资源（如内存，L3缓存）。
